execute as @s[tag=staffstatus,scores={wbmtoggle=1}]  run scoreboard players set BDXdummy Border_Coord_X 20000
execute as @s[tag=staffstatus,scores={wbmtoggle=1}]  run scoreboard players set BDXdummy Border_Coord_Z 20000
...
execute as @s[tag=staffstatus]  run tellraw @a[tag=staffstatus,scores={wbmtoggle=1}] {"rawtext":[{"text":"§¶§cUAC §¶§b► §6World Border §bhas been set to §220k §bby §d"},{"selector":"@s"}]}
...
scoreboard players set @s lstcmd 28