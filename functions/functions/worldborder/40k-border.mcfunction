execute as @s[tag=staffstatus,scores={wbmtoggle=1}]  run scoreboard players set BDXdummy Border_Coord_X 40000
execute as @s[tag=staffstatus,scores={wbmtoggle=1}]  run scoreboard players set BDXdummy Border_Coord_Z 40000
...
execute as @s[tag=staffstatus]  run tellraw @a[tag=staffstatus,scores={wbmtoggle=1}] {"rawtext":[{"text":"§¶§cUAC §¶§b► §6World Border §bhas been set to §240k §bby §d"},{"selector":"@s"}]}
...
scoreboard players set @s lstcmd 30