execute as @s[scores={mdmtoggle=1}]  run execute @s[tag=staffstatus] ~~~ scoreboard players set mdmtoggledummy emeraldmd 1
...
execute as @s[scores={mdmtoggle=1}]  run execute @s[tag=staffstatus] ~~~ tellraw @a[tag=staffstatus] {"rawtext":[{"text":"§¶§cUAC §¶§b► §6Emerald Alerts §bhas been toggled §2ON §bby §d"},{"selector":"@s"}]}
