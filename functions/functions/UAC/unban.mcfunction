execute as @s[tag=!PermBan]  run tellraw @a {"rawtext":[{"text":"§¶§cUAC ► §d"},{"selector":"@s"},{"text":" §¶§bWas §2unbanned §bby an §dOperator"}]}
execute as @s[tag=PermBan]  run tellraw @a {"rawtext":[{"text":"§¶§cUAC ► §d"},{"selector":"@s"},{"text":" §¶§cis UAC Global Banned and can't be unbanned"}]}
...
execute as @s  run function UAC/warnreset
tag @s remove BanCBE
tag @s remove Ban
