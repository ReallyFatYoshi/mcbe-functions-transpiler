...
execute as @p[scores={cleararea=2},tag=staffstatus]  run tellraw @a {"rawtext":[{"text":"§¶§cUAC §b► §d"},{"selector":"@s"},{"text":" §¶§bjust cleared a small area"}]}
kill @e[type=armor_stand,name=cleararea]

#Make-Shift "are you sure"
execute as @p[scores={cleararea=1},tag=staffstatus]  run tellraw @p[tag=!clearcheck] {"rawtext":[{"text":"§¶§cUAC► This will fill all blocks within a 15 radius with air!"}]}
execute as @p[scores={cleararea=1},tag=staffstatus]  run tellraw @p[tag=!clearcheck] {"rawtext":[{"text":"§¶§cUAC► This can't be undone! Execute command again if you're sure."}]}
execute as @p[scores={cleararea=2},tag=staffstatus]  run tellraw @p[tag=!clearcheck] {"rawtext":[{"text":"§¶§cUAC► The immediate area has been cleared with air."}]}
...
execute as @p[scores={cleararea=2}]  run scoreboard players reset @s cleararea
