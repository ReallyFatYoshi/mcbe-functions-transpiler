execute as @s[tag=staffstatus]  run execute @s[scores={ibmtoggle=1}] ~~~ scoreboard players set BNAdummy BNA 0
execute as @s[tag=staffstatus]  run execute @s[scores={ibmtoggle=1}] ~~~ scoreboard players operation @a BNA = BNAdummy BNA
execute as @s[tag=staffstatus]  run execute @s[scores={ibmtoggle=1}] ~~~ scoreboard players set @a BNA 0
...
execute as @s[tag=staffstatus]  run execute @s[scores={ibmtoggle=1}] ~~~ tellraw @a[tag=staffstatus] {"rawtext":[{"text":"§¶§cUAC §¶§b► §6Harming Arrow §bhas been §2ALLOWED §bby §d"},{"selector":"@s"}]}
